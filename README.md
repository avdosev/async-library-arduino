...
Если быть более корректным, то у esp есть процессы выполняющие работу в перерыве между вызовом функции `loop`, во время вызова `delay` или `yield`, тк цикл событий блокирует вызовы функции `loop` то esp думает что мы зависли из-за чего происходит перезагрузка и вывод ошибки
